...
class ColmapDataParserConfig(DataParserConfig):
    """Dataset config for 3D scene data coming from COLMAP"""
    ...
    expand_bboxes: Tuple[float, float, float] = (1.3, 1.3, 1.1)
    """All bboxes will be expanded by these factors in (x, y, z) dimensions to allow for noise in the input data"""
    ...
    frame_select: Optional[List[int]] = field(default_factory=lambda: None)
    """Frame selection for dynamic annotations. Defaults to all frames (changed from original code). """
...
class ColmapDataParser(DataParser):
        ...
        if (self.config.data / f"{split}_list.txt").exists():
            ...
            if self.config.frame_select is not None:
                ...
                if self.config.filter_camera_id:
                    list_included_cameras = [x - 1 for x in self.config.filter_camera_id]
                else:
                    list_included_cameras = range(len(counts.tolist()))
                for i in list_included_cameras:
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        if self.config.load_dynamic_annotations:
            ...
            # get 4x4 matrix
            transform_matrix_colmap = torch.eye(4, dtype=transform_matrix.dtype)
            transform_matrix_colmap[:3,3] = torch.tensor(translation[:3])
            # Load dynamic annotations
            transform_matrix_anno = transform_matrix @ transform_matrix_colmap

            annotations = InterpolatedAnnotationSet.load(
                self.config.data / "annotation.json",
                self.config.data / "aggregate_lidar" / "dynamic_objects",
                scale_factor=scale_factor,
                ignore_static=True,
                transform_matrix=transform_matrix_anno
            )
            metadata.update({"object_annos": annotations})
    ...
    def _setup_downscale_and_undistort(
        ...
        if self._downscale_factor is None:
            if self.config.downscale_factor is None:
                ...
                if self._downscale_factor != 1:
                    CONSOLE.log(f"Using computed image downscale factor of {self._downscale_factor}")
            else:
        ...
        with status(msg="[bold yellow]Downscaling and undistorting images...", spinner="growVertical"):
            camera_list = joblib.Parallel(n_jobs=8, backend="threading")(
                joblib.delayed(self._downscale_and_undistort_one)(
                    cameras[i],
                    filename_pers[i],
                )
                for i in range(len(filenames["image"]))
            )
        cameras = Cameras(
    ...
    def _get_fname(self, parent: Path, filepath: Path, downscale_factor=None, undistort=None) -> Path:
        ...
        if downscale_factor is None:
            downscale_factor = self._downscale_factor or 1.
