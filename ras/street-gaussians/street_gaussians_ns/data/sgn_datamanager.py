...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Migration: removing this hard-coded limit on GPU cacheing
        # This could be added as an option, but it's also possible to select cpu/gpu with an option
        #  so let's not stop the user using lots of GPU memory if they want to
        ...
        if self.config.masks_on_gpu is True:
            if "mask" in self.exclude_batch_keys_from_device:
                self.exclude_batch_keys_from_device.remove("mask")
        if self.config.images_on_gpu is True:
            if "image" in self.exclude_batch_keys_from_device:
    ...
    def cached_train(self) -> List[Dict[str, torch.Tensor]]:
        ...
        images_data = self._load_images("train", cache_images_device=self.config.cache_images)
        for image_data in images_data:
            for key, val in image_data.items():
                if isinstance(val, torch.Tensor):
                    image_data[key] = val.to(self.device)
        return images_data
    ...
    def _load_images(
        ...
        # Put the training cameras on the device to avoid copying frequently
        dataset.cameras = dataset.cameras.to(self.device)
        for camera in dataset.cameras:
            camera.times = camera.times.cpu()
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        # Save time by not copying the image: this takes a long time and we don't modify it anyway
        # These are already on GPU when we get them here
        data = self.cached_train[image_idx]
        #data = dict((k, deepcopy(v)) if k != "image" else (k, v) for (k, v) in data_orig.items())
        # Migration: removed the deepcopy, as it involves a long CPU->GPU copy every step
        #  I don't know why we need to deepcopy, when the data should not be modified in-place
